...
@export var coin_scene: PackedScene # Para asignar la moneda
...
func _physics_process(delta):
	...
	# ✅ Aplica gravedad en todos los estados vivos
	velocity.y += gravity * delta
	velocity.y = clamp(velocity.y, -9999, max_fall_speed)
			...
			if not has_hit:
				for body in attack_area.get_overlapping_bodies():
					if body.name == "Player" and body.has_method("take_damage"):
						...
						has_hit = true
		"hit":
			velocity.x = 0  # ✅ No se mueve durante hit
...
func chase_behavior():
	...
	if attack_area.get_overlapping_bodies().has(player):
		if can_attack:
			...
			velocity.x = 0
			# 👇 Fuerza animación idle si está esperando
	...
	# ✅ Asegúrate de salir de cualquier animación anterior
	if sprite.animation != "run":
		sprite.play("run")
...
func take_damage(amount := 1):
	if state == "dead":
		return  # Ya está muerto, ignorar todo

	# Restar vida pero evitar que baje de 0
	...
	# Si aún está vivo, reproducir hit
	state = "hit"
	sprite.play("hit")
...
func _on_area_2d_body_exited(body: Node2D) -> void:
	if state == "dead":
		return  # ✅ No cambiar estado si ya está muerto
...
func _on_attack_timer_timeout() -> void:
	...
	# ✅ Solo volver a perseguir si no está en otro estado
